...
fn check(ground_truth: &[Contact], res: &[Contact], name: &str){
    ...
    println!("{},{}",ground_truth.len(),res.len());
    ...
    for (p,(contact1, contact2)) in c1.iter().zip(c2.iter()).enumerate() {
        ...
        if !((contact1.0==contact2.0) && (contact1.1==contact2.1)) {
            panic!("{}-th pair mismatch! ground_truth:{},{}; res:{},{}, res{}:{}{}, res{}:{}{}", p, contact1.0, contact1.1, contact2.0, contact2.1,
            p-1, c2[p-1].0, c2[p-1].1, p+1, c2[p+1].0, c2[p+1].1);
        }
    }
    println!("{} passed",name);
}
fn main() {
    let mut hulls = generate_random_hulls(10000, (50, 100), (V3::new(0.0, 0.0, 0.0), V3::new(1.0, 1.0, 1.0)));
   ...
   //check(&c1, &c3, "parry's parallel narrow");
    check(&c3, &c4, "my parallel narrow");
    check(&c3, &c5, "my serial double");
    check(&c3, &c6, "my parallel double");
